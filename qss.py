...
# ── QSS model wrapper with exact unlearning ──


@dataclasses.dataclass
class QSSModel:
    """Trained QSS model with joint-count accumulators.

    S0: (ell, b, K) float64 — raw label sums per level per cell.
    n: (ell, b) int64 — point counts per level per cell.
    N_co: dict (k,m) -> (b, b) int64 — co-occurrence counts.
    mu: list of ell arrays, each (b, K) — materialized predictions.
    """
...
def build_accumulators(
    ...
    """Build additive accumulators from training codes and labels.

    All three quantities are additively decomposable — unlearning
    subtracts from them.
    """
    S0 = np.zeros((ell, b, K), dtype=np.float64)
    n = np.zeros((ell, b), dtype=np.int64)
    N_co: dict[tuple[int, int], np.ndarray] = {}
    for level in range(ell):
        np.add.at(S0[level], (codes[:, level], labels), 1)
...
def rematerialize(
    ...
    """Re-derive per-level predictions from additive accumulators.

    Sequential: level m uses predictions from levels 0..m-1.
    Cost: O(ell^2 * b^2 * K), independent of dataset size N.
    """
    mu = []
    for m in range(ell):
        S_m = S0[m].copy()
...
def qss_train(
    ...
    b: int,
    ell: int,
    rq: faiss.ResidualQuantizer | None = None,
) -> QSSModel:
    """Train a QSS model: RQ codebook + accumulators + materialized predictions.

    Args:
        embeddings: (N, d) float32, should be L2-normalized.
        labels: (N,) int64, 0-indexed class labels.
        K: number of classes.
        b: codebook size per level (power of 2).
        ell: number of RQ levels.
        rq: optional pre-trained RQ codebook.
    """
    if rq is None:
        rq = train_rvq(embeddings, embeddings.shape[1], b, ell)
    codes = encode_rq(rq, embeddings, b, ell)
    S0, n, N_co = build_accumulators(codes, labels, K, b, ell)
    mu = rematerialize(S0, n, N_co, K, b, ell)
    return QSSModel(rq=rq, S0=S0, n=n, N_co=N_co, mu=mu, b=b, ell=ell, K=K)


def qss_model_predict(
    model: QSSModel,
    embeddings: np.ndarray,
) -> np.ndarray:
    """Predict class scores from a QSSModel. Argmax gives predicted class.

    Returns: (M, K) float64 score matrix.
    """
    codes = encode_rq(model.rq, embeddings, model.b, model.ell)
    n = codes.shape[0]
    combined = np.zeros((n, model.K), dtype=np.float64)
    for level in range(model.ell):
        combined += model.mu[level][codes[:, level]]
    return combined
...
def qss_unlearn(
    ...
    """Exact unlearning: remove points by decrementing accumulators.

    The result is bitwise identical to retraining from scratch on
    the remaining data. Cost: O(ell^2 * b^2 * K), independent of N.

    Args:
        model: trained QSSModel.
        codes: (M, ell) int32 codes of points to delete.
        labels: (M,) int64 labels of points to delete.

    Returns: new QSSModel with the points removed.
    """
    S0 = model.S0.copy()
    n = model.n.copy()
    N_co = {k: v.copy() for k, v in model.N_co.items()}

    for level in range(model.ell):
        np.add.at(S0[level], (codes[:, level], labels), -1)
        np.add.at(n[level], codes[:, level], -1)
    for m in range(model.ell):
        for k in range(m):
            np.add.at(N_co[(k, m)], (codes[:, k], codes[:, m]), -1)

    mu = rematerialize(S0, n, N_co, model.K, model.b, model.ell)
    return QSSModel(
        rq=model.rq, S0=S0, n=n, N_co=N_co, mu=mu,
        b=model.b, ell=model.ell, K=model.K,
    )
...
def compute_eta(
    ...
    """Variance explained ratio at one RQ level: eta = Var(E[r|c]) / Var(r)."""
    global_var = np.var(residuals)
    if global_var < 1e-12:
        return 0.0
    cell_sums = np.zeros(b, dtype=np.float64)
    cell_counts = np.zeros(b, dtype=np.float64)
    np.add.at(cell_sums, codes_1d, residuals)
    np.add.at(cell_counts, codes_1d, 1)
    occupied = cell_counts > 0
    cell_means = np.zeros(b, dtype=np.float64)
    cell_means[occupied] = cell_sums[occupied] / cell_counts[occupied]
    total_n = cell_counts.sum()
    weighted_mean = (cell_means * cell_counts).sum() / total_n
    explained_var = (
        (cell_means - weighted_mean) ** 2 * cell_counts
    ).sum() / total_n
    return float(explained_var / global_var)


# ── Storage computation ──


def storage_bytes(b: int, ell: int, K: int) -> dict[str, int]:
    """Compute storage requirements for QSS accumulators in bytes."""
    n_pairs = ell * (ell - 1) // 2
    cooccur = n_pairs * b * b * 8
    raw_label = ell * b * K * 8
    cell_counts = ell * b * 8
    cache = ell * b * K * 8
    return {
        "cooccurrence_bytes": cooccur,
        "raw_label_bytes": raw_label,
        "cell_counts_bytes": cell_counts,
        "cache_bytes": cache,
        "total_bytes": cooccur + raw_label + cell_counts + cache,
    }
